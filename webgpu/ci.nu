...
export def "search reports by-test-name" [
  term: string,
  --mode: string@"nu-complete test-searcher mode" = "substring",
  --in-dir: directory = "../wpt/",
  --include-skipped = false,
] {
  use std/log [] # set up `log` cmd. state
  ...
  $files
    ...
    | flatten
    # FIXME: This doesn't work, because `test` isn't an exact match. Perhaps we need to extract URL
    # params earlier?
    | if $mode == "exact" {
      reject test
    } else {
...
def "test-searcher" [
  ...
  let recognized_mode = match ($recognized_modes | length) {
    1 => {
